...
function validateIPv4subnet ( subnet, description )
    ...
    if not cidr then
        error( "Bad subnet: CIDR value missing for subnet" )
    end
...
function ScanNetworkForHosts ( Subnet )
    ...
    local shellCommand = "nmap -n -sP --max-rtt-timeout 25ms "..thisSubnet
...
function findHostsOnNetwork ( Subnet )
    ...
    if #DiscoveredHosts < 1 then
        error("Scan of network did not return ANY hosts!")
    end
...
function printHostReportJSON (SortedHosts, familiarityTag )
    if #SortedHosts == 0 then
        resultToReturn = string.format( "No %s hosts found.", familiarityTag )
    else
        local parser = require 'cjson'
        resultToReturn = parser.encode(SortedHosts)
    end
end
...
function genNetworkHostsReport ( Subnet,
    ...
    -- Print out the JSON version of the report.
    -- printHostReportJSON(HostsThatAreKnown, isKnownTag )

    printHostReportJSON(HostsThatAreUnknown, isUnknownTag )
end
...
function main ( Database )
    ...
    for _, Subnet in ipairs( Database.Subnets ) do
        ...
        -- if resultToReturn:match("Scan of network did not return ANY hosts!")  then
        --     break
        -- end
...
-- local parser = argparse("hosts-on-network", "Tool to find hosts on the network")
-- parser:option("-v --vendor", "System vendor name.")
-- args = parser:parse()
